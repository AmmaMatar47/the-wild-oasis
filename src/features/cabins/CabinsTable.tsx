...
const initialDeleteInfoState = { cabinId: null, name: '', open: false, imagePath: '' };
...
const CabinsTable = ({
  ...
  const handleDeleteMenuItemClick = (cabinId: number, cabinName: string, cabinImage: string) => {
    setDeleteCabinInfo({ cabinId: cabinId, name: cabinName, open: true, imagePath: cabinImage });
  };

  const handleDeleteCabin = () => {
    if (deleteCabinInfo.cabinId !== null) {
      deleteCabin(deleteCabinInfo.cabinId, deleteCabinInfo.imagePath).then(() => fetchCabins({}));
    }
    setDeleteCabinInfo(initialDeleteInfoState);
  };
  ...
  return cabins.length !== 0 || isLoading ? (
    <>
      ...
      <DeleteDialog
        ...
        onOpenChange={() => setDeleteCabinInfo(initialDeleteInfoState)}
        title='Delete cabin'
